...
def load_image_feature(folder, test):
    ...
    with file_io.FileIO(folder + '/schema', mode='r') as schema_in:
    ...
    with file_io.FileIO(image_feature_path, mode='r') as feature_in:
        image_feature = pd.read_csv(feature_in, header=None, names=schema)
...
def record_cv(
    ...
    with file_io.FileIO(
        ...
        mode='w') as record_output:
            json.dump(record_dict, record_output)
...
if __name__ == '__main__':
    ...
    # Folder for feature pickles and image feature files
    parser.add_option(
        '-p', '--data_folder', action='store', type='string', dest='pickle_folder')
    # Train results folder
    parser.add_option(
        '-p', '--job_dir', action='store', type='string', dest='job_dir')
    ...
    # Set glabal variables (some pathes)
    DATA_FOLDER = options.data_folder
    PICKLE_FOLDER = DATA_FOLDER + 'pickles/'
    IMAGE_FOLDER = DATA_FOLDER + 'image_features/'
    TARGET_PATH = PICKLE_FOLDER + 'deal_probability'
    CV_RECORD_FOLDER = options.job_dir + '/'
